...
class WizardApp(tk.Tk):
    def __init__(self):
        ...
        self.vars = {
            ...
            "author": tk.StringVar(value="Unknown"),
    ...
    def _build_step1_page(self, parent):
        ...
        left = ttk.LabelFrame(page, text="Step 1 — Title / Metadata", padding=12)
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 8))
        left.columnconfigure(1, weight=1)

        right = ttk.LabelFrame(page, text="Notes", padding=12)
        right.grid(row=0, column=1, sticky="nsew", padx=(8, 0))
        right.columnconfigure(0, weight=1)

        note = ttk.Label(
            left,
            text="Enter the app title, description, author, and optional title ID here. You can randomize the title ID with the button beside the field.",
            wraplength=360
        )
        ...
        ttk.Button(left, text="Randomize Title ID", command=self.randomize_title_id).grid(row=4, column=2, sticky="e", pady=4)

        ttk.Label(
            right,
            text="The next step builds the banner and SMDH assets using the metadata from this page.",
            wraplength=320
        ).grid(row=0, column=0, sticky="nw", pady=(0, 10))

        ttk.Label(
            right,
            text="If a title ID is supplied, the script will try to pass it through to the SMDH build command and keep the stored value in the field.",
            wraplength=320
        ).grid(row=1, column=0, sticky="nw", pady=(0, 10))

        ttk.Label(
            right,
            text="Random title IDs are 16 hex characters and are generated automatically with the button on the left.",
            wraplength=320
    ...
    def _build_step2_page(self, parent):
        ...
        left = ttk.LabelFrame(page, text="Step 2 — Banner / SMDH Assets", padding=12)
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 8))
        left.columnconfigure(1, weight=1)
    ...
    def _build_step3_page(self, parent):
        ...
        box = ttk.LabelFrame(page, text="Step 3 — Build CIA", padding=12)
        ...
        ttk.Label(box, text=".cia will be saved in the output folder.").grid(row=4, column=2, sticky="w", pady=4)
        return page
    ...
    def _field_box(self, parent, label, key, row, fixed_value=None, types=None):
        ...
        if fixed_value is None:
            ...
            ttk.Label(parent, text="Auto-filled").grid(row=row, column=2, sticky="w", pady=4)
